...
def pushToClosestForLine(points, centers, deg=0.5, doNorm=False, doNormForPush=False):
    ...
    # for cosine could also try to push so that angle decreses by e.g. 50% - and not euclid dist - but more complicated and no time to check properly
    # and perhaps difference would be marginal
...
if __name__ == '__main__':
    ...
    doNormForPush = True if sys.argv[10] in ("True", "true") else False   # shouldn't really make a difference - Tales theorem
