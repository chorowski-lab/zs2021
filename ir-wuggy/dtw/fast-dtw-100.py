...
def dtw(config):
    ...
    @njit(parallel=True)
    def f(testsample, trainset, trainset_ids, lengths, m):
        res = np.zeros(m)
        for i in prange(m):
            j = trainset_ids[i]
...
def main(args, config):
    ...
    dataPath = pathlib.Path(f"/pio/scratch/1/i290956/zs2021/output/lexical/train-clean-100/dtw-dm2")
    print(f"Data path: {dataPath}")

    M = 100 if args.dev else 100

    test_to_train_ids = np.zeros((len(testset), M), dtype='int32')
        ...
        if os.path.exists(path) and not args.overwrite:
            ...
            with open(path, 'w') as out:
                for index, seq in enumerate(subset):
                    ...
                    bar.update(j)
                    ids = np.concatenate((test_to_train_ids[j, :], np.random.choice(trainset.n, size=500, replace=False)))
                    res = method(seq, trainset.data, ids, trainset.lengths, M+500)
                    I = np.argsort(res)
                    rs = list(res[I]).__str__().replace(' ', '')
                    fs = list(trainset.filenames[ids[I]]).__str__().replace(' ', '')
